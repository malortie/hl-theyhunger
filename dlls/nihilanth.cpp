...
void CNihilanth :: Spawn( void )
{
	...
	if (m_szRechargerTarget[0] == '\0')	std::strcpy( m_szRechargerTarget, "n_recharger" );
	if (m_szDrawUse[0] == '\0')			std::strcpy( m_szDrawUse, "n_draw" );
	if (m_szTeleportUse[0] == '\0')		std::strcpy( m_szTeleportUse, "n_leaving" );
	if (m_szTeleportTouch[0] == '\0')	std::strcpy( m_szTeleportTouch, "n_teleport" );
	if (m_szDeadUse[0] == '\0')			std::strcpy( m_szDeadUse, "n_dead" );
	if (m_szDeadTouch[0] == '\0')		std::strcpy( m_szDeadTouch, "n_ending" );
...
void CNihilanth :: NextActivity( )
{
	...
	if ((pev->health < gSkillData.nihilanthHealth / 2 || m_iActiveSpheres < N_SPHERES / 2) && m_hRecharger == NULL && m_iLevel <= 9)
	{
		...
		std::sprintf(szName, "%s%d", m_szRechargerTarget, m_iLevel );
	...
	if (m_hRecharger != NULL)
	{
		...
		if (flDist < 128.0)
		{
			...
			if (iseq != pev->sequence)
			{
				...
				std::sprintf( szText, "%s%d", m_szDrawUse, m_iLevel );
		...
		if (m_flLastSeen + 5 > gpGlobals->time && flDist < 256 && flDot > 0)
		{
			...
			else 
			{
				...
				else
				{
					...
					std::sprintf( szText, "%s%d", m_szTeleportTouch, m_iTeleport );
					CBaseEntity *pTouch = UTIL_FindEntityByTargetname( NULL, szText );

					std::sprintf( szText, "%s%d", m_szTeleportUse, m_iTeleport );
