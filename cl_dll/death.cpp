...
int CHudDeathNotice :: MsgFunc_DeathMsg( const char *pszName, int iSize, void *pbuf )
{
	...
	std::strncat( killedwith, READ_STRING(), 32 );
	...
	else
	{
		...
		std::strncpy( rgDeathNoticeList[i].szKiller, killer_name, MAX_PLAYER_NAME_LENGTH );
	...
	else
	{
		...
		std::strncpy( rgDeathNoticeList[i].szVictim, victim_name, MAX_PLAYER_NAME_LENGTH );
	...
	else
	{
		...
		if ( !std::strcmp( killedwith, "d_teammate" ) )
	...
	else
	{
		...
		if ( rgDeathNoticeList[i].iSuicide )
		{
			...
			if ( !std::strcmp( killedwith, "d_world" ) )
			{
				ConsolePrint( " died" );
			}
			else
		...
		if ( killedwith && *killedwith && (*killedwith > 13 ) && std::strcmp( killedwith, "d_world" ) && !rgDeathNoticeList[i].iTeamKill )
		{
			ConsolePrint( " with " );
